...
public class StyledTextCompReplace extends org.eclipse.swt.widgets.Dialog {
  ...
  private static Class<?> PKG = StyledTextCompReplace.class; // for i18n purposes, needed by Translator2!!
  ...
  private boolean findText() {
    ...
    /*
     * }else if (text.getSelectionRange().y > searchString.length()) { start = textString.lastIndexOf(searchString,
     * offset - 1); }else { start = textString.lastIndexOf(searchString, offset - text.getSelectionRange().y - 1); }
     */
  ...
  private void replaceText() {
    ...
    text.setText( text.getText().substring( 0, start )
      + replaceText.getText()
      + text.getText().substring( start + text.getSelectionCount() ) );
